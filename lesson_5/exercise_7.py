...
with open('exercise_7.txt', 'r', encoding='utf-8') as file:
    profits = []
    gains = {}
    average_profit = {}
    result = []
    for string in file:
        firm_name, form, gain, costs = string.split(' ')
        profit = int(gain) - int(costs)
        if profit > 0:
            profits.append(profit)
        gains[firm_name] = profit
    average_profit['average_profit'] = mean(profits)
    result.append(gains)
    result.append(average_profit)
with open('exercisr_7.json', 'w', encoding='utf-8') as file:
